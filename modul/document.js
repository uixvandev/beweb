...
const createDocuments = (req, res) => {
  ...
  const data = { ...req.body };
  // Query SQL untuk insert data dan mendapatkan last inserted ID
...
const readDocuments = (req, res) => {
  // buat query sql
  const querySql =
    "SELECT d.*, count(s.document_id)as response FROM documents d LEFT JOIN signature s ON d.id = s.document_id GROUP BY d.id";
...
const readDocumentId = (req, res) => {
  const querySearch =
    'SELECT d.*, s.document_id FROM documents d LEFT JOIN signature s ON d.id = s.document_id WHERE d.id="' +
    req.params.id +
    '"LIMIT 1';
...
const readChekRespon = (req, res) => {
  const querySearch =
    'SELECT * FROM signature s WHERE document_id="' +
